...
async function createSVGComponents(
  ...
  for (const fileName of fileNames) {
    ...
    const newFileName = `${path.basename(fileName, ".svg")}.tsx`;
    const outputPath = path.join(dirs.output, "icons", newFileName);

    // Add to index file even if it exists
    indexFile += `\nexport { ${className}Icon } from './icons/${newFileName.replace(".tsx", "")}'`;
